...
public class Question_04_Manager {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out
					.println("Cant connect to server. "+ioe.getMessage());
			s = null;
			manager = null;
		}
		...
		BufferedReader in = null, manin = null;
		PrintWriter out = null, manout = null;
		...
		try {
			...
			manin = new BufferedReader(new InputStreamReader(manager.getInputStream()));
			manout = new PrintWriter(new OutputStreamWriter(manager.getOutputStream()));
			
			System.out.println("\n\nManager Syntax: \"Manager: COMMAND\"\n"+
			...
			while (stillPlayingGame) {
				...
				// with each command from the user, determine if it is a manager command or a 
				// normal command. Use different buffers depending on the channel
				
				if(messageToSend.contains("Manager: ") || messageToSend.contains("manager: ")){
					
					// run on manager channel
					
					manout.println( messageToSend.substring(9) );
					manout.flush();
					...
					if (serverMessage != null) {
						System.out.println("Manager channel replied: " + serverMessage + "\n");
					}
					
				}else{
					
					// run on normal client channel
